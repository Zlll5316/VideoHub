...
export default function Sidebar({ activeMenu = 'dashboard', onMenuChange }: SidebarProps) {
  ...
  return (
    <div className="w-64 h-screen glass-effect border-r border-slate-800/50 flex flex-col backdrop-blur-xl bg-[#0a0a12] transition-colors duration-300">
      {/* Logo */}
      <div className="p-8 border-b border-slate-800/50">
        ...
        <p className="text-sm text-slate-300 mt-2 font-light">灵感监控中心</p>
      </div>

      {/* 搜索框区域 - Fixed Spacing & Layout */}
      <div className="px-6 mb-6 mt-4">
        <div className="group flex items-center bg-slate-900/50 hover:bg-slate-900/80 border border-white/5 hover:border-white/10 focus-within:border-indigo-500/50 rounded-lg px-3 h-10 transition-all duration-200">
          {/* 搜索图标 */}
          ...
          <input 
            type="text" 
            placeholder="Search..." 
            className="flex-1 bg-transparent border-none outline-none text-sm text-slate-300 ml-2 placeholder:text-slate-600 min-w-0"
          />
          
          {/* 快捷键 Badge - 确保在大容器内部 */}
          <div className="hidden group-hover:flex items-center gap-0.5 px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[10px] font-medium text-slate-500 ml-2">
          ...
          return (
            <motion.button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all relative ${
                isActive
                  ? 'bg-gradient-to-r from-primary-purple/20 to-primary-blue/20 text-white border border-primary-purple/40 shadow-glow-purple'
      ...
      <div className="p-4 border-t border-slate-800/50 space-y-2">
        <motion.button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:text-white hover:bg-slate-800/40 border border-transparent transition-all"
          whileHover={{ scale: 1.02, x: 4 }}
          whileTap={{ scale: 0.98 }}
        >
          <LogOut size={20} />
